...
function fixture(t,enabled=false){
 ...
 const service=new Runtime(config),controls={command:'conductor',maxRuns:1,maxCalls:40,maxReservedTokens:40000000},disposers=[];let observer;
 const ctx={get:name=>name==='teamConductor'?service:controls,inject:(deps,fn)=>{assert.deepEqual(deps,['teamConductor','conductorControlsReady']);observer=fn;},effect:fn=>disposers.push(fn()),logger:{info(){}}};
 apply(ctx,{configPath,markerPath,podUid:'00000000-0000-0000-0000-000000000123'});
 return {dir,config,configPath,markerPath,service,controls,ctx,run:()=>observer(ctx),dispose:()=>disposers.forEach(f=>f()),read:()=>JSON.parse(readFileSync(markerPath,'utf8'))};
}
...
test('wrong class and mismatched configuration are refused',async t=>{const f=fixture(t);f.ctx.get=()=>({});await assert.rejects(f.run(),/refused/);f.ctx.get=name=>name==='teamConductor'?f.service:f.controls;f.service.config.database+='different';await assert.rejects(f.run(),/refused/);assert.equal(existsSync(f.markerPath),false);});
...
test('preflight rejection produces no readiness',async t=>{const f=fixture(t);f.service.preflight=async()=>{throw Error('test preflight failed');};await assert.rejects(f.run(),/test preflight failed/);assert.equal(existsSync(f.markerPath),false);f.dispose();});
test('operator state inside model workspace is refused',async t=>{const f=fixture(t);f.config.workspaceRoot=f.dir;f.service.config.workspaceRoot=f.dir;writeFileSync(f.configPath,JSON.stringify(f.config));await assert.rejects(f.run(),/refused/);assert.equal(existsSync(f.markerPath),false);});
