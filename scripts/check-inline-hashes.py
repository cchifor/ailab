...
The repo carries a couple of inline, hand-computed content-hash values with
no reloader/controller to keep them honest:
...
per mismatch, and exits non-zero if ANY site drifted.
...
def check_platform_dev_job_suffix() -> Site:
    ...
    image_m = re.search(r"^\s*image:\s*(\S+)", text, re.MULTILINE)
    if not image_m:
        raise ValueError(f"container image not found in {job_path}")
...
def main() -> int:
    ...
    for check in SITES:
        ...
        if site.expected == site.actual:
            ...
            print(f"DRIFT {rel} expected={site.expected[:8]} actual={site.actual[:8]}")
            ok = False
