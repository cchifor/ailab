...
| **Decisions (source of truth)** | [`docs/decisions/`](docs/decisions/) — ADRs 0001–0023, living documents; the newest, [0023](docs/decisions/0023-searxng-shared-with-strive.md), admits the strive platform's search services to the dsh SearXNG through one selector-AND'ed `from` element on `searxng-allow` |
...
| K8s: dsh agent UI | ✅ done — **DeepSeek Harness** at `dsh.chifor.me` (Cloudflare Access-gated), running the estate's own models through LiteLLM. Toolchain on `node:22` (bash/python3/git — alpine has no bash and every Bash tool call failed). Agent sandboxed to its own `/workspace` volume, off the config directory. **web_search** via a self-hosted **SearXNG** in-namespace, so the pod that executes model-authored code keeps **no internet egress** — DNS + LiteLLM + SearXNG only; `web_fetch` deliberately off (see runbook). Providers ship as files, not npm plugins — a bare specifier crash-loops the pod ([`docs/runbooks/dsh.md`](docs/runbooks/dsh.md)) |
...
| AgentForge v2 (agentic dev platform) | ✅ **activated + proven on tenant-zero** (2026-07-21) — credential-injecting **broker** (per-account Anthropic/codex OAuth injection, capability-JWT verify) + ephemeral **Kata sandbox** executor + **codex cross-review gate**, with KEDA scale-to-zero and ESO+OpenBao secret sync; real plans + `granted` codex critiques live end-to-end (ADR 0018/0019, `docs/runbooks/agentforge.md` + `agentforge-activation.md`) |
