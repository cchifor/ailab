...
| K8s: dsh agent UI | ✅ done — **DeepSeek Harness** at `dsh.chifor.me` (Cloudflare Access-gated), running the estate's own models through LiteLLM. Toolchain on `node:24` (bash/python3/git/curl — alpine has no bash and every Bash tool call failed), plus **`kubectl` v1.31.4** installed at pod start. Agent sandboxed to its own `/workspace` volume, off the config directory. **web_search** via a self-hosted **SearXNG** in-namespace; since 2026-09-09 the pod also has public IPv4 HTTP(S) egress with private ranges excluded. **Its ServiceAccount is `cluster-admin`** since 2026-09-18 (ADR 0025) — the agent operates the ailab cluster directly; the network exclusions bound only its direct connections. Providers ship as files, not npm plugins — a bare specifier crash-loops the pod ([`docs/runbooks/dsh.md`](docs/runbooks/dsh.md), [`docs/runbooks/dsh-k8s-admin.md`](docs/runbooks/dsh-k8s-admin.md)) |
