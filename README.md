...
| K8s application HA (#100) | 🔄 in progress — tiered availability (ADR 0016): Tier A ×2 + PDB/spread (cloudflared ✅; infra-pg CNPG, Grafana→Postgres, Authelia+auth-valkey in flight), Tier B accepted singletons documented; `ha-rules` guardrail alerts; `docs/runbooks/node-maintenance.md` (out-of-service-taint fast path); strive-pg ×3 spec → `cchifor/platform` (`docs/superpowers/specs/2026-07-06-strive-platform-ha-tier1-spec.md`) |
