...
- **(b) The codex CLI needs a `/v1` base_url AND a forced, policy-allowed model.** codex uses
  ...
  `capability policy rejected`. **Since 2026-09-05 the estate codex model is `gpt-6-astra`** (GPT-6
  Astra): the kid-policy allowlist entitles it, `cross_review.model` + the cross-reviewer role in
  `cchifor/agentforge-config` pin it, and the sandbox/orchestrator images ship codex ≥ 0.153.1 —
  the upstream rejects an older CLI for this model with `400 … requires a newer version of Codex`,
  so the image bump must land BEFORE the config flip.
