...
## 14. Infrastructure Reporting dashboard + Proxmox metrics — ✅ DONE (2026-06-16)
One Grafana dashboard "ailab — Infrastructure Reporting" (uid `ailab-reporting`, generated by
`scripts/gen-reporting-dashboard.py` → `reporting-dashboard.yaml`, 36 panels, collapsible rows:
**Overview / Hypervisors / Instances (VMs+CTs) / AI / Storage**). Enabled by **prometheus-pve-exporter**
(`pve-exporter.yaml` + SOPS `pve-exporter-secret.sops.yaml`; read-only PVEAuditor token
`prometheus@pve!exporter`) — the bare-metal Proxmox hosts previously had **no metrics at all**. Exposes
cluster-wide `pve_*` (single label `id` = `node/…` | `qemu/…` | `lxc/…` | `storage/<node>/<pool>`); the
ServiceMonitor scrapes one host at `/pve?target=192.168.0.2` (the API returns cluster-wide). Plan **and**
implementation validated by codex; every panel's PromQL spot-checked live. **QNAP detailed metrics
deferred** (SNMP fragile on QuTS hero; a Container-Station node_exporter wouldn't expose disk health) — the
Storage section uses the existing fabric `probe_success`/`probe_duration_seconds` + `kubelet_volume_stats`
(k8s PVCs); future option = node_exporter on QNAP Container Station. **Open loose end:** the live Grafana
...
→ true host CPU/mem/load/disk/network (96 cores, 187 GiB). Codified as **ansible role `node_exporter`**
(`just node-exporter`, site.yml tag `monitoring`) — rebuildable. **Instances**
