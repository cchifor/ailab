...
|---|---|---|
| **SEED** — self-heals from `seeds.json` | `operator/dispatcher/forge` (`AF_BOT_TOKENS`), `operator/conductor/forge` (`AF_BOT_TOKENS`), `operator/reaper/ledger` (`AF_REAPER_LEDGER_DSN`), `operator/broker/{anthropic/claude-max-1,anthropic/claude-max-2,anthropic/claude-max-3,anthropic/claude-max-4,openai/codex-pro}/ledger` (all five `AF_BROKER_LEDGER_DSN`, one shared `agentforge_broker` DB on infra-pg), `operator/ci/runner-registration` (`token`), `operator/ci/scaler-token` (`token`) | **Nothing for the KV** — re-provision restores them. (The broker-ledger + CI paths were added on 2026-07-26 and were RESCUE before that. The ledger set has GROWN since: three broker ledgers existed at that commit — `claude-max-{1,2}` plus `codex-pro` — `claude-max-3` landed later the same day and `claude-max-4` on 2026-09-05. Count the broker manifests, never this sentence.) **But a seeded forge PAT is only as fresh as its last commit:** both `*/forge` paths carry a Gitea PAT, which a wipe does not revoke, so what comes back is whatever `operator-seeds.sops.yaml` last recorded. If a PAT was rotated without re-committing the seed, re-provision restores a **revoked** credential and the consumer 401s — re-mint and re-seed rather than debugging the consumer. |
| **RESCUE** — from the Retain'd Secret | `operator/broker/{anthropic/claude-max-1,anthropic/claude-max-2,anthropic/claude-max-3,anthropic/claude-max-4,openai/codex-pro}/oauth` — **all FIVE**, confirmed against the live vault 2026-09-13 | Re-seed from the Retain'd k8s Secret via the recipe in step 5. **Cannot be seeded** (`cas_required=true`) unless `bootstrap.py` is changed to CAS-write. Codex (`codex-pro/oauth`) **also rotates nightly** (single-use refresh token) so it stays a rescue regardless. **This row said THREE until 2026-09-13**, having been written when three brokers existed; a recovery that followed it would have left `claude-max-3` and `claude-max-4` with no OAuth credential while every step reported success. Derive the list from the broker manifests (or `bao kv list af/operator/broker/...`), never from this cell. |
...
| **SEED (estate subtree)** — a THIRD, independent seed path | `estate/{proxmox,qnap,cloudflare,registry,gitea,github,restic,platform,oauth,strive-realm}` (`docs/runbooks/openbao-estate-credentials.md` — that page's table is the list; don't trust this copy of it over that one) | **Nothing** — the daily `openbao-estate-provision` Job seed-patches these from `estate-seeds.sops.yaml`, and its fail-closed matrix covers every path/field named here. Nothing logs in against them (escrow), so there is no credential to re-mint. **But the subtree is wider than the seed:** `estate/{litellm,unifi,wifi}` are live in KV with no seed and no matrix entry, so a wipe loses them silently and this row does NOT cover them. `estate/litellm` also has one reader — policy `af-app-dsh`. |
| **PUBLISHER-OWNED** — not seeded, and not restorable by any seeder (ADR 0026) | `dsh/credentials.DSH_CODEX_*`, `litellm/chatgpt.*` — the reviewer-2 seats' access tokens, projected every minute by `dsh-codex-publisher` on that host (`docs/runbooks/dsh.md` § Codex subscriptions) | **LOST until the publisher's next successful minute, and the order matters.** These are access tokens that the reviewer host renews every few days; a seed would put a stale token back daily, so there is none. Sequence: **(0)** restore the k8s-auth role `af-app-dsh` BY HAND first — `bao write auth/kubernetes/role/af-app-dsh bound_service_account_names=dsh-eso bound_service_account_namespaces=dsh token_policies=af-app-dsh token_ttl=1h alias_name_source=serviceaccount_uid` — because the unchanged `openbao-dsh-provision` Job only ASSERTS that role and aborts without it (a pre-existing gap ADR 0026 records rather than fixes; the chatgpt Job needs no such step, it writes its own role). **(1)** the `openbao-chatgpt-provision` and `openbao-dsh-provision` Jobs (both `openbao-breakglass-token`-auth, both in this directory) recreate both policies (`dsh-codex-publisher`, `af-app-litellm`), the k8s-auth role `af-app-litellm`, and both documents with their EMPTY keys — `af/litellm/chatgpt` with its four `CHATGPT_*` fields + `CHATGPT_OPENBAO_CANARY`, `af/dsh/credentials` with ONLY its `DSH_OPENBAO_CANARY`; that document's other operator fields (forge PAT, operator SSH) follow the ceremonies in `docs/runbooks/dsh.md`. **(2)** The AppRole ceremony in `dsh.md` recreates AppRole `dsh-codex-publisher` and mints a NEW role-id/secret-id into `/etc/dsh-codex-publisher/approle.json` on reviewer-2 — the wipe invalidated the old one, exactly as it does the dev-worker secret-ids. **(3)** The publisher's next minute fills both documents. It can only **PATCH** (no put, no create), so (1) MUST come first or every run fails on a 404; and a SOFT-DELETED document is refused (`deletion_time` set on the current version) until an operator `bao kv undelete`s it or the Job's loud-failure branch ("neither readable nor creatable") is resolved. ESO Retain keeps dsh and litellm serving the last-good token throughout — until it expires (~7 days), which is the real deadline. Verify with `dsh_codex_projection_ok == 1` for both documents. |
| **SYNC-OWNED** — a FOURTH precedence: not seeded, and not restored by any seeder | `dev-workers/<hostname>.tep_kubeconfig`, `dev-workers/<hostname>.helmtest_kubeconfig` (ADR 0021), `dev-workers/<hostname>.platform_kubeconfig` (ADR 0028, same CronJob) and `dev-workers/<hostname>.platform_pg_{user,password,valid_until}` (ADR 0028, a SECOND CronJob: `openbao-platform-pg-sync` in ns `strive-ailab`, which needs the k8s-auth role `platform-pg-sync` from the same daily provision Job and re-creates the Postgres roles itself — so it self-heals from a wipe with no operator action, and a worker missing those fields simply renders no platform credentials) | **LOST until a successful sync, and the order matters.** These are cluster-derived (TokenRequest against the `tep-dw<N>` / `helmtest-dw<N>` ServiceAccounts) and deliberately absent from `devworker-seeds.sops.yaml` — a seeded copy would be re-applied daily by the seed-wins loop and revert every worker to a stale bearer token. Sequence: **(1)** vault + `af` mount + **both** k8s-auth roles and the policy they share — `k8stoken-sync` and `platform-pg-sync`, all three created by the daily `openbao-devworker-provision` Job; **(2)** run **both** syncs and confirm each one's own success line — `openbao-k8stoken-sync` prints `validated N/N fields` (3 fields x live slots, 15/15 for the current five) and `openbao-platform-pg-sync` prints `published N/N slots`; **(3)** re-mint every AppRole secret-id (the row above); **(4)** ONLY THEN restart the workers' agents. Restarting before (2) is not merely early — `error_on_missing_key` + `exit_on_retry_failure` make a missing field exit the agent, so it sits in a restart loop with `~/.git-credentials` unmaintained. **And note what a wipe does NOT change: the agent config on disk.** A worker that had platform access keeps its `platform` stanzas in `/etc/openbao-agent/agent.hcl` across the wipe — the ansible gate only decides what a *future* run writes — so the pg-sync is not optional cleanup here; until it has published, that worker's agent exits on the missing field and takes `~/.git-credentials` with it. If both syncs cannot be restored promptly, stop the agent or remove those stanzas rather than leaving it to restart-loop. |
...
`provisioner-deploy`) all run the orchestrator image and are covered by the repin in step 1. Four
...
- **`openbao-dsh-provision`** (same directory, `dsh-provision-job.yaml`) — owns policy `af-app-dsh`
  and creates `af/dsh/credentials` once (canary only; the contents are operator-owned). Breakglass
  token, `optional: false`; no seeds Secret.
- **`openbao-chatgpt-provision`** (same directory, `chatgpt-provision-job.yaml`, ADR 0026) — owns
  policies `af-app-litellm` and `dsh-codex-publisher` and the k8s-auth role `af-app-litellm`, and
  creates `af/litellm/chatgpt` once (empty `CHATGPT_*` keys + canary; the contents are the
  reviewer-2 publisher's). Breakglass token, `optional: false`; no seeds Secret. These two are what
  step (1) of the PUBLISHER-OWNED row above depends on.
...
5. **Re-seed the RESCUE-class paths** — all FIVE broker `oauth` (every SEED-class path self-heals from
   ...
   **The field name is provider-specific** — `CLAUDE_CODE_OAUTH_TOKEN` for **all four** anthropic
   `claude-max-{1,2,3,4}` brokers, `auth.json` for `openai/codex-pro` (the ESO extract copies the whole doc, so
   the wrong key would sync a wrong-shaped Secret). Field names read back from the live vault 2026-09-13;
   this said "the two anthropic `claude-max-{1,2}`" while four existed. Then re-stamp `bao kv metadata put -cas-required=true
...
2. Parse `stringData.seeds.json` → add the entries (preserve the existing ones) → re-serialize.
   **Two levels, two different serializations, and they are easy to swap:** the seeds BLOB is
   `json.dumps(obj, indent=2)` inside a `|` literal block scalar indented by 8; an individual VALUE
   that is itself JSON — `AF_BOT_TOKENS`, say — is a compact `json.dumps(v, separators=(',',':'))`
   **string** inside it. Getting this backwards reformats the whole document, which buries the one
   line that actually changed. **The `|` costs you a trailing newline**: a YAML parser hands back the
   scalar *with* one and `json.dumps` never emits one, so compare against
   `json.dumps(obj, indent=2) + "\n"` if you parsed with YAML, or against `json.dumps(obj, indent=2)`
   if you de-indented the block's lines yourself and dropped the trailing blank. Prove it instead of
   trusting this paragraph: re-serialize the blob *before* editing it and assert the result is
   byte-identical to what you parsed; only then add the key. An assertion that fails here is telling
   you your newline convention is wrong, not that the document is.
3. Re-encrypt. Prefer `sops edit` driven by a **programmatic `EDITOR`** — sops shlex-splits `$EDITOR`
   and appends the decrypted tmpfile as the **LAST** argument, after every argument you configured
   (use forward slashes; shlex eats Windows backslashes). With `EDITOR="python shim.py"` that lands
   at the shim's `sys.argv[1]`, but only because the shim took no arguments of its own — read it as
   `sys.argv[-1]` if yours does. A shim can then fetch the value from its live source itself, so the
   plaintext never
