...
- The Authelia session (12h) **outlives** the per-app Access token (30m–24h), so Access re-auth becomes
  a **silent redirect** rather than a login. That is what lets the sensitive apps keep 30m windows.
