...
take the OPTIONAL stronger quiesce before dumping: `flux --context admin@ai suspend kustomization
...
`kubectl --context admin@ai -n af-tenant-tenant-zero-platform-dev scale deploy/af-orch-platform-dev-delivery
--replicas=0` (the tenant namespace — `agentforge` holds the CP itself plus the
dispatcher/reaper, but not the tenant orchestrators; the same
...
but it is the whole tenant plane, and the resume below is what ends that pause. Scale back up and `flux --context admin@ai resume kustomization agentforge-tenants` at the step-6 restart —
