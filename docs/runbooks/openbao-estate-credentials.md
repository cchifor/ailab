...
| `af/estate/strive-realm` | `keycloak_admin_user`, `keycloak_admin_password`, `e2e_worker_usernames`, `e2e_worker_password`, `load_persona_username`, `load_persona_password`, `ops_admin_username`, `ops_admin_password` | live cluster Secrets in ns `strive-ailab`: `keycloak-secrets/admin-password`, `e2e-secrets/worker-password`, `load-secrets/persona-password`, `ops-admin-secrets/persona-password` | **the e2e and load persona passwords: in no git repo** — `e2e-secrets` and `load-secrets` are hand-created (no Flux labels, no owner); the e2e one has ONE more home, the worker projection `strive_test_password` in `devworker-seeds.sops.yaml` → `af/dev-workers/common` (docs/runbooks/openbao-dev-workers.md), so its rotation touches that file too; **`keycloak_admin_password` and `ops_admin_password`: a SECOND home each** — the platform repo's `deploy/secrets/ailab/keycloak-secrets.enc.yaml` and `deploy/secrets/ailab/ops-admin-secrets.enc.yaml` (Flux SOPS → the live Secrets), so rotation must touch that file AND this seed in the same change, or the escrow silently goes stale (the Job writes the vault only — it cannot revert the live credential). `ops_admin_*` (added 2026-09-22) is the dedicated App-upgrade administrator `ops-admin@localhost` (realm role `admin`, operator tenant) behind the platform repo's `scoped-app-upgrade.yml` lane; its password has a THIRD consumer home, the Gitea Actions secret `STRIVE_ADMIN_TEST_PASSWORD` on `cchifor/platform` (variable `STRIVE_ADMIN_TEST_USER`), set by hand through the Gitea API — rotate all three |
