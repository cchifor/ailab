...
| `af/estate/registry` | `ci_password`, `oidc_client_secret`, `pull_password`, `trueswarm_ci_password` | `ansible/secrets/registry.sops.yaml` | same SOPS file (zot htpasswd + OIDC); the OIDC secret's pbkdf2 **hash** is separately committed in `kubernetes/apps/apps/auth/authelia-config.yaml`; `pull_password` (read-only user `pull`) is also projected to dev-workers as `af/dev-workers/common` `registry_pull_password` — rotate both; `trueswarm_ci_password` (zot user `trueswarm-ci`, push to `trueswarm/**` only) has a second home, the `cchifor/trueswarm` Gitea Actions secret `REGISTRY_PASSWORD` — rotate both |
...
| `af/estate/strive-realm` | `keycloak_admin_user`, `keycloak_admin_password`, `e2e_worker_usernames`, `e2e_worker_password`, `load_persona_username`, `load_persona_password`, `ops_admin_username`, `ops_admin_password` | live cluster Secrets in ns `strive-ailab`: `keycloak-secrets/admin-password`, `e2e-secrets/worker-password`, `load-secrets/persona-password`, `ops-admin-secrets/persona-password` | **the load persona password: nowhere else at all; the e2e persona password: ONE more home, the SOPS-encrypted worker projection `strive_test_password` in `devworker-seeds.sops.yaml` → `af/dev-workers/common` (docs/runbooks/openbao-dev-workers.md) — rotate it there too** — `e2e-secrets` and `load-secrets` themselves are hand-created (no Flux labels, no owner, in no git repo); **`keycloak_admin_password` and `ops_admin_password`: a SECOND home each** — the platform repo's `deploy/secrets/ailab/keycloak-secrets.enc.yaml` and `deploy/secrets/ailab/ops-admin-secrets.enc.yaml` (Flux SOPS → the live Secrets), so rotation must touch that file AND this seed in the same change, or the escrow silently goes stale (the Job writes the vault only — it cannot revert the live credential). `ops_admin_*` (added 2026-09-22) is the dedicated App-upgrade administrator `ops-admin@localhost` (realm role `admin`, operator tenant) behind the platform repo's `scoped-app-upgrade.yml` lane; its password has a THIRD consumer home, the Gitea Actions secret `STRIVE_ADMIN_TEST_PASSWORD` on `cchifor/platform` (variable `STRIVE_ADMIN_TEST_USER`), set by hand through the Gitea API — rotate all three |
...
> is **per path, not per field**: `read` on `af/estate/strive-realm` hands over all eight fields,
...
> of an estate-class credential — and `af/dev-workers/common.strive_test_{user,password}`
> (the e2e persona, added 2026-09-12 on `dev-worker-3`, moved to `common` 2026-09-25) is the
> second. Projection also keeps the blast radius of a
