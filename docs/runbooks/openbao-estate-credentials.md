...
| `af/estate/registry` | `ci_password`, `oidc_client_secret`, `pull_password` | `ansible/secrets/registry.sops.yaml` | same SOPS file (zot htpasswd + OIDC); the OIDC secret's pbkdf2 **hash** is separately committed in `kubernetes/apps/apps/auth/authelia-config.yaml`; `pull_password` (read-only user `pull`) is also projected to dev-workers as `af/dev-workers/common` `registry_pull_password` — rotate both |
