...
- **Credentials.** Issuing new credentials revokes nothing: an AppRole *periodic* token (72 h,
  ...
  **revocation** (SecretID accessors destroyed, issued token accessors revoked by role, role + policy
  ...
  rewrites the `k8stoken-sync` policy (later in the same script). So the gate **fences** first —
  `kubectl -n openbao patch cronjob openbao-k8stoken-sync -p '{"spec":{"suspend":true}}'` and no
  provision Job in flight — then the provision run narrows the sync policy *before* it deletes the
  retired KV (the script order), then the CronJob is resumed, and a second provision run plus one
  sync run prove the retired paths stay absent.
...
6. **OpenBao (breakglass, `openbao-dev-workers.md`).** The declarative revocation already ran if the
   ...
   Job is re-run the runbook's way — `kubectl -n openbao delete job openbao-devworker-provision`,
   `flux reconcile kustomization openbao`, wait for the **new Job UID** to complete (the Job is a
   standalone `batch/v1 Job`, not a CronJob: `kubectl create job --from` does not apply, and a
   ConfigMap-only change never re-runs a completed Job) after checking the merged script/seed
   revisions are the ones mounted; explicit sync runs use the real CronJob
   (`kubectl -n openbao create job --from=cronjob/openbao-k8stoken-sync <name>`). Then the negative
   test: the old
...
- Secrets for the reassigned slots: the **OpenBao SecretIDs are minted before merge and committed
  encrypted in C2** — against the *destination* roles `dev-worker-3` / `dev-worker-4` (role_ids
  belong to the roles and do not move between YAML keys; the old pairs are not re-keyed). The
  **tep tokens cannot be**: `tep-dw3`/`tep-dw4` are re-created with new UIDs at the gate (below),
  which invalidates every token minted before that, new or old, and the `tep-dwN-token` Secrets in
  `tep-access.yaml` (type `kubernetes.io/service-account-token`) are deleted with their SA and
  re-populated by the token controller for the new UID. The tokens the hosts actually use are
  rendered by `openbao-k8stoken-sync` into OpenBao after the recreation; the SOPS fallback
  (`ansible/secrets/tep-tokens.sops.yaml`, consumed by `scripts/tep-render-kubeconfigs.py`) is
  regenerated **after** the new UIDs exist, in a small follow-up PR-C2b, or retired once both hosts
  are proven to render from OpenBao — C2 says which.
- The slot-3/4 artifacts that stay: `dw3`/`dw4` Cloudflare + tunnel + homepage + monitoring targets
  (`.10`/`.11` unchanged), `helmtest-dw3`/`dw4` namespaces and their **guarded** SAs. Recorded
  exception: the guarded `helmtest-dw3`/`dw4` SAs are not replaced, so 4204 (the new dw3) keeps a
  valid `helmtest-dw4` token until its 30-day expiry — the only holder of a token for a slot it no
  longer owns is a trusted survivor whose rendered kubeconfig the converge overwrites; the expiry
  date is written into the gate record.
...
4. **Re-IP 4204 → slot 3.** Credential-consuming services quiesced first and **kept quiescent across
   both reboots**: `systemctl disable --now openbao-agent.service` (the installed unit — the role's
   `tasks/openbao.yml`), the same for `herdr.service` on the pilot host and any claude-job unit, then
   `systemctl mask` them so an enabled unit cannot come back with the old role/config at boot;
   verified stopped after each reboot. They are released only in step 6 — `systemctl unmask`
   immediately before the limited converge, whose own start tasks/handlers then start them with the
   destination identity. Then, over SSH on `.11`: add `.10` as a second address, `netplan
   ...
   and a hard abort deadline — if C2 has not merged by then, step 4 is reversed (rollback B below)
   before the silence expires; if it merged but did not reconcile or apply in time, rollback B2.
5. **Merge PR-C2**, then the barrier in two halves. **5a** — the manifests: `flux get kustomizations`
   Ready at the merge sha; slot 5 pruned everywhere (`helmtest-dw5`, `tep-dw5` + its token Secret,
   the k8stoken-sync objects); both connectors rolled; no `.12` targets; the sync CronJob suspended
   and the provision Job's new UID completed (slot-5 revocation logged). **5b** — the identities that
   the destructive apply must not race: `tep-dw3` and `tep-dw4` SAs deleted and re-created by Flux
   (new UIDs invalidate every earlier TokenRequest token, including 4203's, and the token controller
   re-fills `tep-dw3-token`/`tep-dw4-token`), the CronJob resumed and one explicit sync run
   (`create job --from=cronjob/openbao-k8stoken-sync`) validated at **8 fields**. Only then
   `tofu apply` of the regenerated dev-workers plan (destroy 4203; `name` on 4204/4205) and of the
   cloudflare plan (`dw5`).
6. **Identities.** Retired slot 5: the negative login test and KV gone (the revocation ran in 5a).
   Reassigned slots 3/4: the *previous* SecretID accessors of roles `dev-worker-3` and
   ...
   every worker); the old tep tokens denied and the new ones accepted (a `kubectl --token` `auth
   can-i` each way, nothing printed); then `systemctl unmask` and the converge **limited to the two
   re-slotted hosts** (`--limit dev-worker-3,dev-worker-4`) renders the new AppRole login, tep
   token, helmtest kubeconfig, `agentforge.env`, Caddy site and ttyd title and starts
   `openbao-agent`/herdr with the destination identity; the estate credential's destination
   (Context) applied; the daily converge re-enabled; PR-C2b (the SOPS tep fallback) opened.
...
### Gate G3a-2 record (2026-09-23, dev-worker-3's original VM 4203 + slot 5; re-slot 4204 → slot 3, 4205 → slot 4)

- 2026-09-23T08:11:12Z rehearsal on a state copy (worktree config @ 22294bca): `state mv` dev-worker-3->retired-4203, dev-worker-4->dev-worker-3, dev-worker-5->dev-worker-4; `tofu plan` = **0 to add, 2 to change (`name` only: 4204 dev-worker-4->dev-worker-3, 4205 dev-worker-5->dev-worker-4), 1 to destroy (retired-4203 = vmid 4203 on ai-node3)**. State copies deleted after the rehearsal.

- 2026-09-23T08:23:48Z pre-flight facts (read-only): 4204 (.11) host key ED25519 SHA256:hWFdSY6nam6cCmBFrVb43jnv/wdPlHIZCN9zE0q1390, MAC bc:24:11:0d:b3:c2, openbao-agent enabled+active, no herdr; 4205 (.12) host key ED25519 SHA256:+/SNuVph6t5lfKDY7eisFk8RP1JvfcFZZoVHW3nG+6Q, MAC bc:24:11:0a:9a:95, openbao-agent + herdr enabled+active; 4203 (.10) MAC bc:24:11:b2:2a:c2 (ai-node1 ARP shows .10 owned by it). Both survivors: netplan 50-cloud-init.yaml matches by MAC with a static address, 99-disable-network-config.cfg present, /etc/hosts 127.0.1.1 <name>.lan <name>. Limitations found: no serial0 and guest agent disabled on all three VMs (no `qm terminal`, no `qm agent ping` — console fallback is the PVE web console), `arping` absent on the hosts (the ".10 has no owner" proof = `ip neigh` + ping after 4203 stops). Staged copies under /c/Users/chifo/work/home/_copyoff/g3a2-prep-20260923T082310Z.

- 2026-09-23T08:45:43Z SecretIDs minted before merge (PR-C2 credential commit):
  - dev-worker-3: NEW accessor f9680634-a71e-86a4-d8bd-235e14c3487b (keep); previous holder accessor(s) ['01c40900-83e9-9e75-373c-19d905f97feb'] (destroy at step 6)
  - dev-worker-4: NEW accessor 9e1f4317-726b-1f6b-4192-f7c246306546 (keep); previous holder accessor(s) ['4924a141-c98b-fae1-c011-0894c3e14f7a'] (destroy at step 6)

- 2026-09-23T08:48:43Z step 2: `qm shutdown 4203` (ACPI; PVE reported a wait timeout but the guest completed the orderly shutdown ~08:48:10Z), `stopped`, `onboot 0`; .10 answers no ping from ai-node1 afterwards.

- 2026-09-23T09:03:14Z step 4 done: 4204 = dev-worker-3/.10 (rebooted 08:53Z, back with REGENERATED SSH host keys — `qm cloudinit update` after the ipconfig0/name change gives cloud-init a new instance-id, so it re-runs first-boot modules incl. ssh key generation; identity confirmed by MAC bc:24:11:0d:b3:c2 + machine-id 99603588, new key SHA256:54k74ukXzKaUJESxiuAj8l/vKnvy0RysyGC3Cjd7Zzc pinned); 4205 = dev-worker-4/.11 (rebooted 09:01Z, MAC bc:24:11:0a:9a:95, machine-id 2f256401, keys regenerated likewise). openbao-agent (both) and herdr (4205) stopped+disabled; `systemctl mask` refused (unit files under /etc/systemd/system). .12 silent. Old netplans kept at /root/50-cloud-init.yaml.before-G3a-2 on both.

- 2026-09-23T09:05:57Z pre-apply plans regenerated after the in-guest renames: dev-workers `0 add / 0 change / 1 destroy` (retired-4203; the `name` updates were already applied by `qm set --name`, saved `_out/dev-workers.G3a-2.tfplan`); cloudflare full plan shows an UNRELATED pending change (`cloudflare_dns_record.tunnel["router"]` import + update from the operator's PR c963b6f5, never applied to the local state) — the gate applies a TARGETED plan with only the two dw5 destroys (`_out/cloudflare.G3a-2.dw5.tfplan`); the router import is left for the operator. Sync CronJob suspended 09:03Z; `no-automerge` removed 09:03Z, both reviewers approved at ee017852.

- 2026-09-23T09:13:49Z steps 5–6 done: PR #828 merged as db84560e (09:09Z); Flux at db84560e everywhere (helmtest, testpool, openbao, apps, edge-connector, infrastructure, platform-access); helmtest-dw5 / tep-dw5 (+token) / platform-dw5 gone; cloudflared at config-revision 2026-09-23-retire-dw5 2/2; homepage tile gone; no .12 Prometheus targets; provision Job re-run (new UID eb1bc60c…, 09:08Z): `retired dev-worker-5: 1 secret-id accessors destroyed, 2 tokens revoked, role/policy deleted`, KV metadata deleted, dev-worker-6 converged. 5b: tep-dw3/tep-dw4 SAs re-created (new UIDs 76a74659… / 13408ea6…) and their token Secrets re-populated; sync CronJob resumed, explicit run `validated 12/12 fields`; pre-gate SOPS tep tokens for slots 3/4 rejected with token-only auth (slot 1 still accepted). tofu apply dev-workers: 4203 destroyed, plan = No changes; ai-node3 has no 4203 VM/LVs/config, MemAvailable 40.66 GiB at 09:11Z (start of the 24 h floor window, node_memory_MemAvailable_bytes{instance="192.168.0.4:9100"}). cloudflare targeted apply: dw5 DNS + Access destroyed; authoritative NS: dw5 NXDOMAIN, dw3/dw4 NOERROR; the unrelated router import stays for the operator. Rotation: previous-holder accessors 01c40900… (slot 3) and 4924a141… (slot 4) destroyed, 5 + 2 issued tokens revoked; logins: old pairs for slots 3/4/5 → 400 invalid, new pairs → 200 (test tokens revoked). V-C2 so far: qm names/balloons right (4204 dev-worker-3 12288, 4205 dev-worker-4 6144), .12/.13 silent, nodename labels dev-worker-3@.10 / dev-worker-4@.11, no NodeExporterDown, Kyverno denies an admin delete of helmtest-dw3 SA, dw3/dw4.chifor.me → Access 302, ansible-inventory = 4 hosts. Limited converge of dev-worker-3,dev-worker-4 running from the WSL clone at db84560e (WSL known_hosts pinned to the MAC-verified keys).

- 2026-09-23T09:19:55Z converge: the first limited run failed on both hosts at "Check that the vault is reachable under this host's own AppRole" (`cred list` needs the agent token; the play-end handler that starts the agent never ran) — `systemctl enable --now openbao-agent` by hand on both (agents logged in as dev-worker-3 / dev-worker-4, `cred list` OK), second run failed=0 (dev-worker-3 ok=176 changed=7, dev-worker-4 ok=190 changed=8): ttyd titles dev-worker-3/dev-worker-4, Caddy + ttyd active, herdr enabled+active on dev-worker-4, `cred exec` reads the own subtree and another slot's is denied. `~/.tep/kubeconfig` is owned by the SOPS fallback fleet-wide (`dev_worker_openbao_kubeconfig_cutover: false`, no host carries `/etc/openbao-agent/renders-kubeconfigs`), so the converge from main deployed the PRE-gate tokens for slots 3/4 (401) — PR-C2b carries the regenerated `tep-tokens.sops.yaml`; `tep` on dev-worker-3/4 works after its merge + a `-t tep` converge. `~/.helmtest/kubeconfig` does not exist on any host by design until that cutover, so the plan's helm smoke is N/A (the helmtest-dw3/dw4 SAs, RBAC and Kyverno guard are unchanged and verified). Daily converge task re-enabled 09:23Z (next run 09-24 06:35 local); silences left to expire 10:46Z.
- 2026-09-23T09:29Z closed: PR-C2b (#831) merged as d00ce38f; the limited converge from that main
  (`failed=0`, `changed=1` per host) re-rendered `~/.tep/kubeconfig` on dev-worker-3/4 — `tep list`
  works from both, `auth whoami` = `system:serviceaccount:testpool:tep-dw3` / `tep-dw4`, `can-i create
  sandboxclaims` yes, `list pods` yes, `delete pods` in kube-system no. **Deviation from V-C2:** the
  `tep` acquire + release smoke was NOT run — a lease consumes the pool's only warm member, which
  is the parent plan's soaking member (`env-std-pool-nw6n4`, epoch 0′ = 09-21 12:36Z); the RBAC
  proof above covers the same identity without rotating it. The helm smoke is N/A until the
  kubeconfig cutover (no host renders `~/.helmtest/kubeconfig`). Gate G3a-2 complete; the 24 h floor
  window for G3b runs from 09:11Z (`node_memory_MemAvailable_bytes{instance="192.168.0.4:9100"}`).
