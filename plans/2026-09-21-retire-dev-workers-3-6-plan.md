...
- 2026-09-23T09:29:40Z closed: PR-C2b (#831) merged as d00ce38f; the limited converge from that main (`failed=0`, `changed=1` per host) re-rendered `~/.tep/kubeconfig` on dev-worker-3/4 — `tep list` works from both, `auth whoami` = `system:serviceaccount:testpool:tep-dw3` / `tep-dw4`, `can-i create sandboxclaims` yes, `list pods` yes, `delete pods` in kube-system no. **Deviation from V-C2:** the `tep` acquire + release smoke was NOT run — a lease consumes the pool's only warm member, which is the parent plan's soaking member (`env-std-pool-nw6n4`, epoch 0′ = 09-21 12:36Z); the RBAC proof above covers the same identity without rotating it. The helm smoke is N/A until the kubeconfig cutover (no host renders `~/.helmtest/kubeconfig`). Gate G3a-2 complete; the 24 h floor window for G3b runs from 09:11Z (`node_memory_MemAvailable_bytes{instance="192.168.0.4:9100"}`).
