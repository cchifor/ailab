...
| **epoch reset** (not a check-in) | 2026-09-21 12:14 → 12:36Z | — (a host event, not the guest freeze under test) | **NotReady 12:19:00 → 12:36:00Z**, boot 12:34:23Z — `ai-node2` was hard power-cycled by the operator's node-maintenance session (its previous boot ends in `crash` in `last -x`; ai-node3 had been rebooted 11:39Z the same way), which took every guest on it down with the host | `EnvNodeKubeletMetricsBlocked` firing 12:29→12:36Z; `TestpoolNoWarmCapacity` / `PVCStuckPending` pending | 0 reaps, 0 closures (the guest died with its host, nothing to catch); reaper DaemonSet pod re-created 12:35:02Z | `env-std-pool-chjqs` lost at 4 h 14 m; replacement `k6546` Pending 12:19→12:36Z (PVC stuck while the node was down), then **`env-std-pool-nw6n4` created 2026-09-21 12:36:00Z = the member that is soaking** | **Epoch 0′ = 2026-09-21 12:36:00Z.** Marks: 24 h = 09-22 12:36Z, 66 h = 09-24 06:36Z, day 7 = 09-28 12:36Z. The day-1 check-in was not run on 09-22. |
| day 1 (late) | 2026-09-21 08:20 → 2026-09-23 06:51Z (run 09-23 ~07:00Z from `main`) | PREVENTION-FAILED **as printed** = the epoch-reset row above (17 m NotReady, 20 m capacity gap > 10 m bound) — a host power-cycle, not a recurrence; after 12:36Z: Ready throughout, no alert, 1 capacity blip 08:21→08:22Z (the V2 injection, contained) | see above | see above | 0 `reap stage=` lines, 272 heartbeats, 0 watchdog closures; the run's relay query over the 46 h window **OOM-killed `loki-0`** (1 GiB, 06:59:22Z) and then reported the relay absent — false: the relay shipped ~20k lines/h all along (Loki `count_over_time` re-checked per hour); the script now fetches in bounded slices (PR: soak-loki-window) | `nw6n4` at 42 h 15 m, 0 restarts; 24 h mark passed with no incident | Checkpoint not written (problems). Next: day 3 after 09-24 06:36Z with the fixed script, from epoch 0′. |
| day 1′ (epoch 0′, late) | 2026-09-21 12:40 → 2026-09-23 09:36Z (run 09-23 ~09:36Z from `main` with the bounded fetch of #829; checkpoint written) | **OK** | Ready throughout (the one boot, 12:34:23Z, precedes the window) | none | 0 reaps, 265 heartbeats, 0 closures, 0 `stop_*` errors; relay: 2697 of ~2696 one-minute buckets, 0 stale freshness slices, 4 untimestamped relay diagnostics and no evidence-class line (a quiet member) — the first run flagged those 4 as "none carries an in-window source time"; the rule is narrowed to replayed-history-only in this PR | `env-std-pool-nw6n4` 45 h 00 m, 0 restarts — past the 24 h mark with no incident; 66 h = 09-24 06:36Z | The window starts 12:40Z, after the host-event refill (an incident already underway at the first sample is classified against an earlier window, so 12:36Z cannot be the check-in start). Next: day 3 after 09-24 06:36Z from the checkpoint, day 7 = 09-28 12:36Z. |
