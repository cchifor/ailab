#!/usr/bin/env bash
###############################################################################
# Provision an ai-llm LXC: llama.cpp (Vulkan/RADV, gfx1151) as a non-root
# systemd service + node_exporter + amdgpu sysfs metrics. Idempotent.
...
CTX="${CTX:-32768}"
PARALLEL="${PARALLEL:-4}"
# Extra llama-server flags. Default empty => mmap ON (weights still go to VRAM via -ngl 99,
# so decode speed is unaffected; mmap's reclaimable file pages are safe under the cgroup cap).
# For the 120B/122B models post-carve-change, pass EXTRA_ARGS="--no-mmap".
...
echo "== [6/7] systemd: llama-server (Vulkan) with warm-up =="
...
[Unit]
Description=llama.cpp server (Vulkan/RADV gfx1151) — ai-llm
...
ExecStartPost=/usr/local/bin/llama-warmup.sh 8080 ${MODEL_ALIAS}
Restart=on-failure
RestartSec=5
...
echo "== [7/7] systemd: amdgpu sysfs metrics -> node_exporter textfile =="
